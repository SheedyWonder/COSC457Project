...
public class SupplierDAO {
    ...
    public void addSupplierDAO(SupplierDTO supplierdto) {
        try{
                String query = "SELECT * FROM suppliers WHERE Name='"+supplierdto.getName()+"' AND location='"+supplierdto.getLocation()+"' AND phone='"+supplierdto.getPhone()+"'";
    ...
    public void editSupplierDAO(SupplierDTO supplierdto) {
            try {
                String query = "UPDATE suppliers SET suppliercode=?,Name=?,location=?,phone=? WHERE suppliercode=?";
    ...
    public ResultSet getQueryResult() {
        try {
            String query = "SELECT suppliercode AS SupplierCode, Name AS Name, location as Address, phone AS Phone FROM suppliers";
            rs = stmt.executeQuery(query);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public ResultSet getSearchSuppliersQueryResult(String searchTxt) {
        try {
            String query = "SELECT suppliercode AS SupplierCode, Name AS Name, location as Address, phone AS Phone FROM suppliers WHERE Name LIKE '%"+searchTxt+"%' OR location LIKE '%"+searchTxt+"%' OR suppliercode LIKE '%"+searchTxt+"%' OR phone LIKE '%"+searchTxt+"%'";
            rs = stmt.executeQuery(query);
        } catch (SQLException e) {
            e.printStackTrace();
        }
