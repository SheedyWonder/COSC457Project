...
public class ProductDAO {
    ...
    public String getSupplierCode(String suppliersName){
        try{
            String query="SELECT suppliercode FROM suppliers WHERE Name='"+suppliersName+"'";
            rs=stmt.executeQuery(query);
            while(rs.next()){
    ...
    public String getCustomerCode(String customersName){
        try{
            String query="SELECT customercode FROM customers WHERE Name='"+customersName+"'";
            rs=stmt.executeQuery(query);
            while(rs.next()){
    ...
    public boolean checkStock(String productcode){
        try{
            String query="SELECT * FROM inventory where productcode='"+productcode+"'";
...
//    addPurchaseDAO
     public void addPurchaseDAO(ProductDTO productdto){
        try {
            ...
            if(checkStock(productCode)==true){
                try {
                    String q = "UPDATE inventory SET quantity=quantity+? WHERE productcode=?";
                    pstmt = (PreparedStatement) con.prepareStatement(q);
    ...
    public void editStock(String val,int q){
        try{
            String query="SELECT * FROM inventory WHERE productcode = '"+val+"'";
            rs=stmt.executeQuery(query);
            if(rs.next()){
                String qry = "UPDATE inventory SET quantity=quantity-? WHERE productcode=?";
    ...
    public void deleteStock(){
         try{
             String q="DELETE FROM inventory WHERE productcode NOT IN(SELECT productcode FROM purchaseinfo)";
    ...
    public void sellProductDAO(ProductDTO productDTO,String username){
        ...
        try{
            String query="SELECT * FROM inventory WHERE productcode='"+productDTO.getProductCode()+"'";
            rs=stmt.executeQuery(query);
            while(rs.next()){
                productCode=rs.getString("productcode");
            ...
            if(productDTO.getQuantity()>quantity){
                ...
                try{
                    String q="UPDATE inventory SET quantity=quantity-'"+productDTO.getQuantity()+"' WHERE productcode='"+productDTO.getProductCode()+"'";
                    stmt.executeUpdate(q);
    ...
    public ResultSet getQueryResultOfInventory() {
        try {
            String query = "SELECT inventory.productcode,products.productname,inventory.quantity,products.costprice,products.sellingprice FROM inventory INNER JOIN products ON inventory.productcode=products.productcode";
            rs = stmt.executeQuery(query);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public String getProductsSupplier(int id){
        String sup=null;
        try {
            String query = "SELECT Name FROM suppliers INNER JOIN purchaseinfo ON suppliers.suppliercode=purchaseinfo.suppliercode WHERE purchaseid='"+id+"'";
            rs = stmt.executeQuery(query);
            if(rs.next()){
                sup=rs.getString("Name");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
